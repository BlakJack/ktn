...
function canTalk(user, room, connection, message) {
    ...
    if (room && room.modchat) {
        if (room.modchat === 'crash') {
            ...
            if (!user.autoconfirmed && (room.auth && room.auth[user.userid] || user.group) === ' ' && room.modchat === 'autoconfirmed') {
                ...
                return false;
            } else if (config.groups.byRank.indexOf(userGroup) < config.groups.byRank.indexOf(room.modchat)) {
    ...
    if (typeof message === 'string') {
        ...
        if (room && room.id === 'lobby') {
            ...
            if ((normalized === user.lastMessage) && (Date.now() - user.lastMessageTime) < MESSAGE_COOLDOWN) {
